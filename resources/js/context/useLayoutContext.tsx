...
const LayoutProvider = ({ children }: ChildrenType) => {
    ...
    const storageKey = '__OSEN_CONFIG__v2';
    ...
    const [settings, setSettings] = useLocalStorage<LayoutState>(storageKey, INIT_STATE, override);
    ...
    useEffect(() => {
        try {
            window.localStorage.removeItem('__OSEN_CONFIG__');
        } catch (error) {
            console.error(error);
        }
    }, []);
