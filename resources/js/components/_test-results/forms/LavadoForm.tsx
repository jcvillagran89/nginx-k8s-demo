...
const TABLE_FIELD_DEFINITIONS: Array<{ id: TableFieldId; candidates: string[] }> = [
  {
    id: 'acetato',
    candidates: [
      'transferencia de color (acetato)',
      'transferencia color (acetato)',
      'transferencia de color acetato',
      'acetato',
      'color transfer (acetate)',
      'transfer acetate',
    ],
  },
  {
    id: 'algodon',
    candidates: [
      'transferencia de color (algodon)',
      'transferencia color (algodon)',
      'transferencia de color algodón',
      'transferencia color algodón',
      'transferencia de color algodon',
      'algodon',
      'algodón',
      'color transfer (cotton)',
      'transfer cotton',
    ],
  },
  {
    id: 'poliamida',
    candidates: [
      'transferencia de color (poliamida)',
      'transferencia color (poliamida)',
      'transferencia de color poliamida',
      'poliamida',
      'color transfer (polyamide)',
      'transfer polyamide',
    ],
  },
  {
    id: 'poliester',
    candidates: [
      'transferencia de color (poliester)',
      'transferencia color (poliester)',
      'transferencia de color poliéster',
      'transferencia color poliéster',
      'transferencia de color poliester',
      'poliester',
      'poliéster',
      'color transfer (polyester)',
      'transfer polyester',
    ],
  },
  {
    id: 'acrilico',
    candidates: [
      'transferencia de color (acrilico)',
      'transferencia color (acrilico)',
      'transferencia de color acrílico',
      'transferencia color acrílico',
      'transferencia de color acrilico',
      'acrilico',
      'acrílico',
      'color transfer (acrylic)',
      'transfer acrylic',
    ],
  },
  {
    id: 'lana',
    candidates: [
      'transferencia de color (lana)',
      'transferencia color (lana)',
      'transferencia de color lana',
      'lana',
      'color transfer (lane)',
      'color transfer (wool)',
...
const LavadoForm: React.FC<SectionFormProps> = ({
  testId,
  sectionName,
  sectionData,
  readOnly,
  ...
  const acetatoKey = tableFields.acetato?.key;
  const algodonKey = tableFields.algodon?.key;
  const poliamidaKey = tableFields.poliamida?.key;
  const poliesterKey = tableFields.poliester?.key;
  const acrilicoKey = tableFields.acrilico?.key;
  const lanaKey = tableFields.lana?.key;
        ...
        <Form onSubmit={isReadOnly ? (e) => e.preventDefault() : handleSubmit}>
          ...
          <div className="table-responsive">
            <Table bordered className="align-middle text-center table-sm table-tight">
              <thead>
                <tr>
                  <th>Transferencia de color (Acetato)</th>
                  <th>Transferencia de color (Algodón)</th>
                  <th>Transferencia de color (Poliamida)</th>
                  <th>Transferencia de color (Poliéster)</th>
                  <th>Transferencia de color (Acrílico)</th>
                  <th>Transferencia de color (Lana)</th>
                  <th>Observaciones</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>{renderGradeField(acetatoKey)}</td>
                  <td>{renderGradeField(algodonKey)}</td>
                  <td>{renderGradeField(poliamidaKey)}</td>
                  <td>{renderGradeField(poliesterKey)}</td>
                  <td>{renderGradeField(acrilicoKey)}</td>
                  <td>{renderGradeField(lanaKey)}</td>
