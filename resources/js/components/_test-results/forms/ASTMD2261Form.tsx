...
const specimenRegex = /^especimen_(\d+)_(urdimbre|trama)(?:_(?:lbf|lb_f|lb\/f))?$/i;


const HEADER_FIELDS: HeaderFieldDefinition[] = [
  {
    id: 'humedad_relativa',
  ...
  },
  {
    ...
    candidates: ['promedio (urdimbre)', 'promedio urdimbre', 'average (warp)', 'average warp'],
    type: 'text',
  },
  {
    id: 'promedio_trama',
    label: 'Promedio (Trama)',
    candidates: ['promedio (trama)', 'promedio trama', 'average (weft)', 'average weft'],
    type: 'text',
  },
  {
    id: 'notas',
...
const ASTMD2261Form: React.FC<SectionFormProps> = ({ testId, sectionName, sectionData, readOnly }) => {
  ...
  const headerFields = useMemo(() => {
    return HEADER_FIELDS.map((definition) => {
      const match = fieldEntries.find(([_, field]) => {
        ...
        return (
          matchesCandidate(dn, definition.candidates) ||
          matchesCandidate(lb, definition.candidates)
        );
      });
  ...
  // Detecta keys por espécimen (urdimbre/trama)
  const specimenFields = useMemo(() => {
    const map: Record<number, { urdimbre?: string; trama?: string }> = {};
    const allowedKeys = new Set<string>();

    fieldEntries.forEach(([key, field]) => {
      // usualmente label ya trae el slug, pero por seguridad intentamos con label y display_name
      ...
      const kind = match[2].toLowerCase() as 'urdimbre' | 'trama';
  ...
  const recomputeAverages = (nextFields: Record<string, string>) => {
    ...
    for (let i = 1; i <= specimenCount; i++) {
      ...
      const w = row.urdimbre ? parseNumber(nextFields[row.urdimbre] ?? '') : null;
      const t = row.trama ? parseNumber(nextFields[row.trama] ?? '') : null;
          ...
          <div className="mb-4">
            <Row className="g-3">
              {headerFields.map(({ definition, key, field }) => {
                const isAvg = definition.id === 'promedio_urdimbre' || definition.id === 'promedio_trama';
                const isNotas = definition.type === 'textarea';

                return (
                  <Col xs={12} sm={isNotas ? 12 : 6} md={isNotas ? 12 : 4} lg={isNotas ? 12 : 3} key={key}>
                    <Form.Group controlId={`${sectionName}-${key}`}>
                      ...
                      {definition.type === 'select' ? (
                        ...
                        <Form.Control
                          type="text"
                          value={data.fields[key] ?? ''}
                          onChange={(e) => !isAvg && handleChange(key, e.target.value)}
                          readOnly={isAvg}
                          disabled={isReadOnly}
                          isInvalid={!!errors[`fields.${key}`]}
                          placeholder={isAvg ? 'Auto' : ''}
                        />
                      )}
